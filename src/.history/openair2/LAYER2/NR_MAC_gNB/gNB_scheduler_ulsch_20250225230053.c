...
int init_ul_slice_policies() {
  const char *config_path = "/home/lyg/oais_first0224/targets/PROJECTS/GENERIC-NR-5GC/CONF/slice_ul_config.json";
  
  FILE *fp = fopen(config_path, "r");
  if (!fp) {
    LOG_E(NR_MAC, "Cannot open slice config file %s\n", config_path);
    return -1;
  }

  struct json_object *parsed_json = json_object_from_file(config_path);
  struct json_object *slice2;
  if (json_object_object_get_ex(parsed_json, "slice2", &slice2)) {
    struct json_object *min_ratio, *max_ratio;
    json_object_object_get_ex(slice2, "min_ratio", &min_ratio);
    json_object_object_get_ex(slice2, "max_ratio", &max_ratio);
    
    // 更新全局数组
    ul_slice_policies[1].min_ratio = json_object_get_int(min_ratio);
    ul_slice_policies[1].max_ratio = json_object_get_int(max_ratio);
  }
