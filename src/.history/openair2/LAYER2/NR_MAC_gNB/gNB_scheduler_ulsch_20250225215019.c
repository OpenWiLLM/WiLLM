...
// 示例中硬编码策略，实际可通过 json 文件初始化全局配置
typedef struct {
  int slice_id;
  int sub_slice_id; 
  int min_ratio; // 比例（百分比）
  int max_ratio;
} ul_slice_policy_t;

// 添加函数来加载和计算切片策略
static void load_slice_policies_from_json(const char *json_file) {
  // 用于临时存储策略的数组
  ul_slice_policy_t temp_policies[20];
  int policy_count = 0;

  struct json_object *root = json_object_from_file(json_file);
  if(!root) {
    LOG_E(NR_MAC, "Error loading JSON file %s\n", json_file);
    return;
  }

  // 读取主切片策略
  struct json_object *rrm_policies;
  json_object_object_get_ex(root, "rrmPolicyRatio", &rrm_policies);
  
  // 读取子切片策略
  struct json_object *sub_policy;
  json_object_object_get_ex(root, "subSlicePolicy", &sub_policy);
  struct json_object *sub_slices;
  json_object_object_get_ex(sub_policy, "subSlices", &sub_slices);

  // 遍历主切片
  for(int i = 0; i < json_object_array_length(rrm_policies); i++) {
    struct json_object *policy = json_object_array_get_idx(rrm_policies, i);
    int slice_id = json_object_get_int(json_object_object_get(policy, "sD"));
    int main_min = json_object_get_int(json_object_object_get(policy, "min_ratio")); 
    int main_max = json_object_get_int(json_object_object_get(policy, "max_ratio"));

    // 查找该主切片下的子切片
    for(int j = 0; j < json_object_array_length(sub_slices); j++) {
      struct json_object *sub = json_object_array_get_idx(sub_slices, j);
      int parent = json_object_get_int(json_object_object_get(sub, "parent_slice"));
      
      if(parent == slice_id) {
        // 找到匹配的子切片,计算最终比例
        int sub_id = json_object_get_int(json_object_object_get(sub, "sub_slice_id"));
        int sub_min = json_object_get_int(json_object_object_get(sub, "min_ratio"));
        int sub_max = json_object_get_int(json_object_object_get(sub, "max_ratio")); 

        // 计算子切片实际PRB比例 - 主切片比例与子切片比例的乘积
        temp_policies[policy_count].slice_id = slice_id;
        temp_policies[policy_count].sub_slice_id = sub_id;
        temp_policies[policy_count].min_ratio = (main_min * sub_min) / 100;
        temp_policies[policy_count].max_ratio = (main_max * sub_max) / 100;
        policy_count++;
      }
    }
  }
  ...
  // 更新全局策略数组
  memcpy(ul_slice_policies, temp_policies, policy_count * sizeof(ul_slice_policy_t));
...
static ul_slice_policy_t ul_slice_policies[] = {
  {1, 1, 20, 50},
  {2, 1, 30,  60},
  {3, 1, 40, 70}
};

#define NUM_UL_SLICE_POLICIES (sizeof(ul_slice_policies)/sizeof(ul_slice_policy_t))

// 示例：根据 UE 的 rnti 查找对应切片 ID（此处直接通过硬编码匹配，不干预已有上行处理）
// 实际可结合 ueSliceMapping 配置实现（注意 rnti 转换为字符串格式，如 "0x1234"）
// 修改get_ue_target_slice函数
static int get_ue_target_slice(int rnti) {
  load_slice_policies_from_json("/home/lyg/oais_first0224/rrmPolicy_sub.json");

  // 将rnti转换为字符串格式
  char rnti_str[10];
  snprintf(rnti_str, sizeof(rnti_str), "0x%x", rnti);

  struct json_object *root = json_object_from_file("/home/lyg/oais_first0224/rrmPolicy_sub.json");
  if(!root) {
    return 1; // 默认切片
  }

  struct json_object *ue_mapping;
  json_object_object_get_ex(root, "ueSliceMapping", &ue_mapping);
  
  struct json_object *ue_policy;
  if(json_object_object_get_ex(ue_mapping, rnti_str, &ue_policy)) {
    // 找到UE映射
    int slice = json_object_get_int(json_object_object_get(ue_policy, "target_slice"));
    int sub_slice = json_object_get_int(json_object_object_get(ue_policy, "target_sub_slice"));
    json_object_put(root);
    
    // 在ul_slice_policies中查找对应的策略
    for(int i = 0; i < NUM_UL_SLICE_POLICIES; i++) {
      if(ul_slice_policies[i].slice_id == slice && 
         ul_slice_policies[i].sub_slice_id == sub_slice) {
        return slice;
      }
    }
  }
...
// // 新增函数：对 UE 的上行 RB 分配进行切片约束调整
// void nr_ul_slice_preprocess(module_id_t module_id, frame_t frame, sub_frame_t slot, uint16_t max_rbSize)
// {
//   gNB_MAC_INST *mac = RC.nrmac[module_id]; //没错
//   NR_UEs_t *ue_info = &mac->UE_info; //没错
//   // 假定上行 BWP 已初始化，且字段 BWPSize 表示总 RB 数量 
//   // 在修改rb_alloc之前,保存原始计算的最小RB需求
//   int total_ul_rb = max_rbSize;
//   printf("---------------------------------------------------------------------------------");
//   printf("total_ul_rb = %d\n", total_ul_rb);
//   printf("---------------------------------------------------------------------------------");


//   if (!ue_info || !ue_info->list) {
//     LOG_W(NR_MAC, "UE info or UE list is NULL\n");
//     return;
//   }
  
//   // 确认至少有一个UE
//   if (ue_info->list[0] == NULL) {
//     LOG_W(NR_MAC, "No UEs in the list\n"); 
//     return;
//   }

//   // 到这里没错，并且没有输出，看来有UE

//   // 遍历所有上行调度中的 UE，对每个 UE 的规划 RB 数量进行切片约束
//   UE_iterator(ue_info->list, UE) // 空函数没有问题
//   {
//     // 加强UE指针检查
//     if (!UE) {
//       LOG_W(NR_MAC, "NULL UE in list\n");
//       continue;
//     }
//     NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl; //没错
    
//     // 在修改rb_alloc之前,保存原始计算的最小RB需求
//     int min_required_rb = sched_ctrl->sched_pusch.rbSize;
    
//     // 使用 sched_pusch 中的 rbSize 作为原始分配值
//     int rb_alloc = sched_ctrl->sched_pusch.rbSize;//没错
//     printf("UE %d: rbSize=%d\n", UE->rnti, rb_alloc);//没错
//     // 如未设置则采用默认分配：本示例全部 RB 数均可调度
//     // if (rb_alloc <= 0)
//     //   rb_alloc = total_ul_rb;

//     int target_slice = get_ue_target_slice(UE->rnti);//没错
//     int min_ratio = 0, max_ratio = 90; //没错
//     // 查找该目标切片对应的上下限约束  
//     for (unsigned int i = 0; i < 3; i++) {
//       if (ul_slice_policies[i].slice_id == target_slice) {
//         min_ratio = ul_slice_policies[i].min_ratio;
//         max_ratio = ul_slice_policies[i].max_ratio;
//         break;
//       }
//     }
//     // // 根据比例计算 RB 数上下界
//     int min_rb = (min_ratio * total_ul_rb) / 90;//没错
//     int max_rbSize = (max_ratio * total_ul_rb) / 90;//没错
//     printf("---------------------------------------------------------------------------------");
//     print("min_rb=%d, max_rbSize=%d\n", min_rb, max_rbSize);
//     printf("---------------------------------------------------------------------------------");
//     return;
...
//     // 对原始分配的 RB 数进行约束修正
//     //  对原始分配的 RB 数进行约束修正
//     // if (rb_alloc < min_rb && rb_alloc > 0) {
//     //   if (UE != NULL) {
//     //     rb_alloc = min_rb;
//     //     printf("UE 0x%04x: rb_alloc %d < min_rb %d, adjusting\n", UE->rnti, rb_alloc, min_rb);
//     //   }
//     // } else if (rb_alloc > max_rb) {
//     //   if (UE != NULL) {
//     //     rb_alloc = max_rb;
//     //     printf("UE 0x%04x: rb_alloc %d > max_rb %d, adjusting\n", UE->rnti, rb_alloc, max_rb);
//     //   }
//     // }
...
//     // printf("UE %d: rbSize=%d\n", UE->rnti, rb_alloc);
//     // // 确保rb_alloc不小于最小RB需求
//     // if (rb_alloc < min_required_rb) {
//     //   LOG_W(NR_MAC, "Slice policy would allocate %d RBs which is less than minimum required %d RBs\n", rb_alloc, min_required_rb);
//     //   rb_alloc = min_required_rb;
//     // }

//     // // const int min_rb_size = 5; // 增大最小RB数

//     // // // 在计算rb_alloc时增加检查
//     // // if (rb_alloc < min_rb_size) {
//     // //   rb_alloc = min_rb_size;
//     // // }

//     // // 在访问 sched_pusch 之前添加检查
//     // if (!sched_ctrl) {
//     //   printf("sched_ctrl is NULL for UE %04x\n", UE->rnti);
//     //   return;
//     // }

//     // if (rb_alloc > 0) {
//     //   sched_ctrl->sched_pusch.rbSize = rb_alloc;
//     //   printf("UE %d: rbSize=%d\n", UE->rnti, rb_alloc);
//     // } else {
//     //   LOG_W(NR_MAC, "Invalid rb_alloc value %d for UE %04x\n", rb_alloc, UE->rnti);
//     //   //多次出现这个情况
//     // }

//     // printf(NR_MAC, "UE 0x%04x (slice %d): final UL RB allocation = %d (total_ul_rb = %d)\n", UE->rnti, 2, rb_alloc, total_ul_rb);

//     //    printf(NR_MAC, "UE 111 (slice %d): final UL RB allocation = %d (total_ul_rb = %d)\n", 2, 0, total_ul_rb);
//   }  //printf("Hello!\n");
// }

// 对min_rb和max_rbSize进行切片约束调整
static void nr_ul_slice_constraint(module_id_t module_id, uint16_t rnti, uint16_t total_rb, uint16_t *min_rb, uint16_t *max_rb)
{
  // 获取当前UE所属的切片ID
  int slice_id = get_ue_target_slice(rnti);

  // 查找该切片的配置
  int min_ratio = 0, max_ratio = 90;
  for (unsigned int i = 0; i < NUM_UL_SLICE_POLICIES; i++) {
    if (ul_slice_policies[i].slice_id == slice_id) {
      min_ratio = ul_slice_policies[i].min_ratio;
      max_ratio = ul_slice_policies[i].max_ratio;
      break;
    }
  ...
  // 根据切片配置计算资源约束
  uint16_t min_rb_slice = (min_ratio * total_rb) / 100;
  uint16_t max_rb_slice = (max_ratio * total_rb) / 100;

  // 调整输入的min_rb和max_rbSize
  if (*min_rb < min_rb_slice) {
    *min_rb = min_rb_slice;
  }

  if (*max_rb > max_rb_slice) {
    *max_rb = max_rb_slice;
  }

  LOG_D(NR_MAC,
        "UE %04x (slice %d): PRB constraints adjusted - min_rb: %d->%d, max_rb: %d->%d\n",
        rnti,
        slice_id,
        *min_rb,
        min_rb_slice,
                              ...
                              const int8_t harq_pid)
{
  ...
  while (!done && pdu_len > 0) {
    ...
    // 在每次上行PDU处理前调用切片预处理函数
    //nr_ul_slice_preprocess(module_idP, frameP, slot);
                  ...
                  uint16_t *rballoc_mask)
{
  ...
  while (remainUEs > 0 && n_rb_sched >= min_rb && iterator->UE != NULL) {
    ...
    max_rbSize = 90;
    // printf("allocating UL data for the UE (rbStart %d, min_rb %d, max_rbSize %d, bwpSize %d)\n", rbStart, min_rb, max_rbSize,
    // bwpSize);
    ...
    nr_ul_slice_constraint(module_id,
      iterator->UE->rnti, 
      max_rbSize,
      &min_rb,
      &max_rbSize);
    ...
    printf("rbSize %d (max_rbSize %d), TBS %d, est buf %d, sched_ul %d, B %d, CCE %d, num_dmrs_symb %d, N_PRB_DMRS %d\n",
           rbSize,
           max_rbSize,
           sched_pusch->tb_size,
           sched_ctrl->estimated_ul_buffer,
...
static bool nr_fr1_ulsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  pf_ul(module_id, frame, slot, nr_mac->UE_info.list, max_sched_ues, len, rballoc_mask);
  return true;
}
