...
struct rb_limits nr_ul_slice_preprocess(module_id_t module_id, frame_t frame, sub_frame_t slot, uint16_t max_rbSize)
{
  ...
  UE_iterator(ue_info->list, UE) // 空函数没有问题
  {
    ...
    // // 根据比例计算 RB 数上下界
    limits.min_rb = (min_ratio * total_ul_rb) / 90;//没错
    limits.max_rb = (max_ratio * total_ul_rb) / 90;//没错
    printf("---------------------------------------------------------------------------------");
