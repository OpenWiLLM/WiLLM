...
// 修改为动态数组
static ul_slice_policy_t *ul_slice_policies = NULL;
static int num_ul_slice_policies = 0;

#define NUM_UL_SLICE_POLICIES (sizeof(ul_slice_policies)/sizeof(ul_slice_policy_t))

// 新增函数用于从json加载配置
static void load_ul_slice_policies(void) {
  // 打开并解析json文件
  FILE *fp = fopen("/home/lyg/oais_first0224/openair2/LAYER2/NR_MAC_gNB/slice_config.json", "r");
  if (!fp) {
    LOG_E(NR_MAC, "Could not open slice config file\n");
    return;
  }
  ...
  json_object_object_get_ex(parsed_json, "ul_slice_policies", &policies);
  
  // 获取策略数量
  num_ul_slice_policies = json_object_array_length(policies);
  
  // 分配内存
  ul_slice_policies = calloc(num_ul_slice_policies, sizeof(ul_slice_policy_t));
  
  // 解析每个策略
  for(int i = 0; i < num_ul_slice_policies; i++) {
    struct json_object *policy = json_object_array_get_idx(policies, i);
    struct json_object *slice_id, *min_ratio, *max_ratio;
    
    json_object_object_get_ex(policy, "slice_id", &slice_id);
    json_object_object_get_ex(policy, "min_ratio", &min_ratio); 
    json_object_object_get_ex(policy, "max_ratio", &max_ratio);
    
    ul_slice_policies[i].slice_id = json_object_get_int(slice_id);
    ul_slice_policies[i].min_ratio = json_object_get_int(min_ratio);
    ul_slice_policies[i].max_ratio = json_object_get_int(max_ratio);
  }
