...
void nr_ul_slice_preprocess(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  UE_iterator(ue_info->list, UE) // 空函数没有问题
  {
    ...
    int min_ratio = 0, max_ratio = 100; //没错
    // 查找该目标切片对应的上下限约束  
    ...
    // // 根据比例计算 RB 数上下界
    int min_rb = (min_ratio * total_ul_rb) / 100;//没错
    int max_rb = (max_ratio * total_ul_rb) / 100;//没错
...
// 在访问 sched_pusch 之前添加检查
    if (!sched_ctrl) {
      printf(NR_MAC, "sched_ctrl is NULL for UE %04x\n", UE->rnti);
      return;
    }
    ...
    if (rb_alloc > 0) {
      ...
      printf(NR_MAC, "Invalid rb_alloc value %d for UE %04x\n", rb_alloc, UE->rnti);
    }
