                 ...
                 slice_schd_t *s_array)
{
  ...
  if (!s_array) {
    ...
    if (remaining_rbs > 0) {
      // 根据最大比例分配剩余资源
      for (int s_i = 0; s_i < num_slice; s_i++) {
        int max_additional = (n_rb_sched * SLI_info->list[s_i]->policy.max_ratio) / 100 - s_array[s_i].min_prbs;
        if (max_additional > 0) {
          int allocated = MIN(remaining_rbs, max_additional);
