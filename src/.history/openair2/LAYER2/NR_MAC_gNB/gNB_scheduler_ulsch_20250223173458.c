...
static bool nr_fr1_ulsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  // 根据时间域分配限制调整可用 PRB
  const int startSymbolIndex = tdaList->list.array[tda]->startSymbolIndex;
  const int nrOfSymbols = tdaList->list.array[tda]->nrOfSymbols;
  const uint16_t symb = SL_to_bitmap(startSymbolIndex, nrOfSymbols);

  // 根据已经被占用的 PRB 调整 n_rb_sched
  int st = 0, e = 0, len = 0;
  for (int i = 0; i < bwpSize; i++) {
  ...
  LOG_D(NR_MAC, 
    "UL scheduling: bwpSize=%d, n_rb_sched=%d, frame=%d slot=%d\n",
    bwpSize, n_rb_sched, frame, slot);
