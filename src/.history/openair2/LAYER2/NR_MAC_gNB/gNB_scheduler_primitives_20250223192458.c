...
void remove_nr_list(NR_list_t *listP, int id) 
{
  // 参数验证
  if (!listP) {
    LOG_E(NR_MAC,"NULL list pointer\n");
    return;
  }

  if (id < 0 || id >= listP->len) {
    LOG_W(NR_MAC,"Invalid id %d (max %d)\n", id, listP->len-1); 
    return;
  }
  ...
  // ID不存在时只记录警告,不触发assertion
  if (*cur == -1) {
    LOG_W(NR_MAC,"ID %d not found in list\n", id);
    return;
  }
  ...
  *next = -1;
  ...
  LOG_D(NR_MAC,"Removed id %d from list\n", id);
}
