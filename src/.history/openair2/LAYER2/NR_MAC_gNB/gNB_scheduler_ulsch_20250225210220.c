...
static const ul_slice_policy_t ul_slice_policies[] = {
  {1, 1, 20, 50},
  {2, 1, 30,  60},
  {3, 1, 40, 70}
};
...
void load_ul_slice_policies_from_json(const char *json_file_path) {
  ...
  struct json_object *rrmPolicyRatio;
  struct json_object *subSlicePolicy;
  struct json_object *ueSliceMapping;

  json_object_object_get_ex(parsed_json, "rrmPolicyRatio", &rrmPolicyRatio);
  json_object_object_get_ex(parsed_json, "subSlicePolicy", &subSlicePolicy);
  json_object_object_get_ex(parsed_json, "ueSliceMapping", &ueSliceMapping);

  size_t num_policies = json_object_array_length(rrmPolicyRatio);
  ul_slice_policy_t *policies = malloc(num_policies * sizeof(ul_slice_policy_t));

  for (size_t i = 0; i < num_policies; i++) {
    struct json_object *policy = json_object_array_get_idx(rrmPolicyRatio, i);
    policies[i].slice_id = json_object_get_int(json_object_object_get(policy, "sST"));
    policies[i].sub_slice_id = json_object_get_int(json_object_object_get(policy, "sD"));
    policies[i].min_ratio = json_object_get_int(json_object_object_get(policy, "min_ratio"));
    policies[i].max_ratio = json_object_get_int(json_object_object_get(policy, "max_ratio"));
  }

  // Update the global ul_slice_policies array
  memcpy(ul_slice_policies, policies, num_policies * sizeof(ul_slice_policy_t));
  free(policies);
  free(json_data);
...
static int get_ue_target_slice(int rnti) {
  char rnti_str[10];
  snprintf(rnti_str, sizeof(rnti_str), "0x%04x", rnti);
  ...
  if (json_object_object_get_ex(ueSliceMapping, rnti_str, &ue_mapping)) {
    int target_slice = json_object_get_int(json_object_object_get(ue_mapping, "target_slice"));
    json_object_put(parsed_json);
    return target_slice;
  }

  json_object_put(parsed_json);
  // Default slice assignment if not found in JSON
  return ul_slice_policies[0].slice_id;
}
...
nr_pp_impl_ul nr_init_fr1_ulsch_preprocessor(int CC_id)
{
   ...
   * not correct*/
  load_ul_slice_policies_from_json("/home/lyg/oais_first0224/rrmPolicy_sub.json");
