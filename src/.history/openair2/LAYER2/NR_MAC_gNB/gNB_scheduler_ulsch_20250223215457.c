...
void nr_ul_slice_preprocess(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  // 注意：如果结构不同，请修改成实际上行 BWP 的字段名
  int total_ul_rb = 0;
  
  // 假设用第一个 UE 的 UL BWP 表示全局上行资源
  if (ue_info->number > 0 && ue_info->list[0]) {
  ...
  UE_iterator(ue_info->list, UE) {
    ...
    // 假设原始上行调度已经在其他流程中计算出一个初始 RB 分配数
    // 此处新增一个字段以容纳该值（例如 sched_ul_rb_size），如果未定义，可先初始化为0或全部 RB。
    int rb_alloc = sched_ctrl->sched_ul_rb_size;  
    ...
    // 更新调度控制中的 RB 分配设置
    sched_ctrl->sched_ul_rb_size = rb_alloc;
