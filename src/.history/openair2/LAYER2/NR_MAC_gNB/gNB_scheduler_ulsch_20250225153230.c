...
void nr_ul_slice_preprocess(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  // 加强指针检查
  if (!mac || !ue_info) {
    LOG_E(NR_MAC, "Invalid MAC or UE_info pointers\n");
    return;
  }
  ...
  if (!ue_info || !ue_info->list) {
    printf(NR_MAC, "UE info or UE list is NULL\n");
    return;
  }
  
  // 确认至少有一个UE
  if (ue_info->list[0] == NULL) {
    printf(NR_MAC, "No UEs in the list\n"); 
    return;
  }
...
// 在访问 sched_pusch 之前添加检查
    ...
    if (rb_alloc > 0) {
      ...
      printf("Invalid rb_alloc value %d for UE %04x\n", rb_alloc, UE->rnti);
      //多次出现这个情况
    }
