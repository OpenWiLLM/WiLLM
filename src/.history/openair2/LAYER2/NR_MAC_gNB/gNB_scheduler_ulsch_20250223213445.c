 ...
 */
 ...
 typedef struct {
  ...
  int max_rb; 
  int priority;
  char schedule_policy[20];
} ul_slice_config_t;

static ul_slice_config_t ul_slice_params[MAX_NUM_SLICES];
static int ul_num_slices = 0;

// 读取RRM策略配置
void read_ul_slice_config() {
  FILE *fp = fopen("rrmPolicy.json", "r");
  if(!fp) {
    LOG_E(MAC, "Cannot open rrmPolicy.json\n");
    return;
  }
  ...
  if(!root) {
    LOG_E(MAC, "Error parsing rrmPolicy.json\n");
    return;
  }

  cJSON *slices = cJSON_GetObjectItem(root, "slices");
  ul_num_slices = cJSON_GetArraySize(slices);

  for(int i = 0; i < ul_num_slices; i++) {
    cJSON *slice = cJSON_GetArrayItem(slices, i);
    ul_slice_params[i].min_rb = cJSON_GetObjectItem(slice, "minRB")->valueint;
    ul_slice_params[i].max_rb = cJSON_GetObjectItem(slice, "maxRB")->valueint;
    ul_slice_params[i].priority = cJSON_GetObjectItem(slice, "priority")->valueint;
    strcpy(ul_slice_params[i].schedule_policy, 
           cJSON_GetObjectItem(slice, "schedulePolicy")->valuestring);
  }
