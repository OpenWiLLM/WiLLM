...
// Move ul_slice_policies declaration to file scope and make it non-const
static ul_slice_policy_t ul_slice_policies[] = {
  {1, 20, 50},
  {2, 30, 60}, // These values will be updated from JSON
  {3, 40, 70}
};

// Add function to update policies from JSON file
void update_ul_slice_policies_from_json(const char *json_file_path) {
  FILE *fp = fopen(json_file_path, "r");
  if (!fp) {
    LOG_E(NR_MAC, "Could not open slice policy JSON file %s\n", json_file_path);
    return;
  }
  ...
  struct json_object *slice_2_min;
  struct json_object *slice_2_max;
  
  parsed_json = json_tokener_parse(buffer);
  if (!parsed_json) {
    LOG_E(NR_MAC, "Error parsing JSON file\n");
    return;
  }

  // Extract values for slice 2
  if (json_object_object_get_ex(parsed_json, "slice_2_min_ratio", &slice_2_min) &&
      json_object_object_get_ex(parsed_json, "slice_2_max_ratio", &slice_2_max)) {
    
    // Update values for slice 2 (index 1 in array)
    ul_slice_policies[1].min_ratio = json_object_get_int(slice_2_min);
    ul_slice_policies[1].max_ratio = json_object_get_int(slice_2_max);
    
    LOG_I(NR_MAC, "Updated slice 2 policies: min_ratio=%d, max_ratio=%d\n",
          ul_slice_policies[1].min_ratio,
          ul_slice_policies[1].max_ratio);
  } else {
    LOG_E(NR_MAC, "Required fields not found in JSON file\n");
  }
