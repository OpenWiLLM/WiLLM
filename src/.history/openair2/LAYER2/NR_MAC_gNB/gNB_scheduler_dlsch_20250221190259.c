 ...
 */
 ...
 void nr_update_slice_policy(module_id_t module_id,
   ...
   for (int i = 0; i < mac->dl_num_slice; i++) {
      printf("Slice_list[%d]->RAN_resource.min_ratio %d, Slice_list[%d]->RAN_resource.max_ratio %d \n",i, SLI_info->list[i]->policy.min_ratio, i,SLI_info->list[i]->policy.max_ratio);
   }
 
 }
 
 // void nr_store_dl_slice_info(module_id_t module_id,frame_t frame, sub_frame_t slot) {
 ...
 //             // Directly assign slice 3 to each logical channel
 //             sched_ctrl->dl_sl_info[lcid].id = 3;
 //         }
 ...
 //         // Assign slice 3 to the available slice list
 //         sched_ctrl->avail_slice_list[1].id = 3;
 //         sched_ctrl->num_slice_d = 2;
 //     }
 // }
       ...
       for(int j=1; j < RC.nrmac[module_id]->dl_num_slice; j++){
         ...
         if(SLI_info->list[j]->conf.nssai_config.sD_flag){
           if((SLI_info->list[j]->conf.nssai_config.sD[2] == sched_ctrl->dl_sl_info[lcid].nssai_config.sD[2])  &&
             ...
             sched_ctrl->dl_sl_info[lcid].id = 1;
           }
 ...
 //original -------------------------------------------------------------------------------------------------------------------------------------------
   ...
   sched_ctrl->avail_slice_list[0].id = 0 	;// slice id = 0 is used when no PDU session is not yet established
   sched_ctrl->num_slice_d=1;
   ...
   for (int k=1;k <= MAX_NUM_PDU_SESSION;k++){
     ...
     if ((slot == 0) && (frame & 127) == 0) {
      printf("The UE is in the slice: %x = [",UE->rnti);
 ...
 void nr_store_dlsch_buffer_slice(module_id_t module_id,
                            frame_t frame,
                            sub_frame_t slot) {
 ...
 //    for (int j=0;j< sched_ctrl->dl_slice_num;j++){
 //  		  if(sched_ctrl->num_total_bytes_slice[j]) printf("*** Number of bytes in slice %d are %d \n", sched_ctrl->slice_id_list[j],sched_ctrl->num_total_bytes_slice[j]);
 //    }
 ...
 //
 //		for(int k=0;k<sched_ctrl->num_slice_d;k++){
 //			if (sched_ctrl-> slice_for_this_sched == sched_ctrl->avail_slice_list[k].id) bytes = sched_ctrl->avail_slice_list[k].bytes;
 //		}
 //
 //		printf ("(UE %x slice %d bytes %u) ", UE->rnti,sched_ctrl-> slice_for_this_sched,bytes );
 //	}
 //	printf("]\n");
 }
 ...
 //
 //  printf("Retransmission for UE %04x, HARQ %d Slice = %d \n",UE->rnti,
 // 		  current_harq_pid,sched_ctrl-> slice_for_this_sched);
     ...
     slice_schd_t *s_array)
 {
   ...
   /* Loop UE_info->list and allocate new transmission */
     UE_iterator(UE_list, UE) {
       ...
       if ( sched_ctrl-> slice_for_this_sched != sl_id){
       //printf("\n In pf_dl sid = %d for slice = %d, but selected slice = %d ** \n",sid,sched_ctrl->active_slice[sid],
       //sched_ctrl-> slice_for_this_sched);
       continue;
       }else{
         //printf(" sched_ctrl-> slice_for_this_sched %d  sl_id) %d \n",sched_ctrl-> slice_for_this_sched,sl_id);
       }
       ...
       uint32_t bytes = 0;
       for(int k=0;k<sched_ctrl->num_slice_d;k++ ){
         if (sched_ctrl->avail_slice_list[k].id == sl_id) bytes = sched_ctrl->avail_slice_list[k].bytes;
       }
        ...
        slice_schd_t *s_array)
 {
   ...
   if (sched_pdsch->dl_harq_pid >= 0) {
     ...
     //printf("Looking to Retransmit for UE %04x  for HARQ %d slot =%d \n",UE_p->rnti,sched_pdsch->dl_harq_pid, slot);
  ...
  */
   ...
   for(int s1=0; s1 < num_slice; s1++){
     ...
     if(s_array[s1].sid == s1) {
       s_array[s1].min_prbs= (n_rb_sched_init * SLI_info->list[s1]->policy.min_ratio )/100;
     }
   ...
   for(int s_i=0;s_i < num_slice ;s_i++){
     ...
     for(int j=0;j< num_slice ;j++){
       if (SLI_info->list[j]->s_id == sid){
         if(max_rb_s > ((n_rb_sched_init * SLI_info->list[j]->policy.max_ratio)/100)){
           max_rb_s = (n_rb_sched_init * SLI_info->list[j]->policy.max_ratio)/100;
         }
       }
     }
     ...
     if (UE->Msg4_ACKed != true)
       ...
       // This UE is already scheduled at previous HARQ stage
     ...
     * (this can be considered a design flaw) */
     ...
     uint32_t bytes = 0;
     for(int k=0;k < sched_ctrl->num_slice_d ;k++){
       if (sid == sched_ctrl->avail_slice_list[k].id) bytes=sched_ctrl->avail_slice_list[k].bytes;
     }
   ...
   while (remainUEs> 0 && n_rb_remain_s >= min_rbSize && n_rb_sched_s <= max_rb_s &&
     ...
     * allocation after CCE alloc fail would be more complex) */
     ...
     nr_find_nb_rb(sched_pdsch->Qm,
             ...
             &TBS,
             &rbSize);
     // Add sub-slice handling here
     bool sub_slice_found = false;
     float priority = 1.0; // Default priority if not found
     int min_rb = 0;
     int max_rb = max_rbSize;

     // Open and parse JSON config
     const char sub_filename[] = "../../../rrmPolicy_sub.json";
     struct json_object *policy_obj = json_object_from_file(sub_filename);

     if (policy_obj)
     {
       // Get sub-slice configuration
       struct json_object *sub_policy_obj, *sub_slices_array;
       if (json_object_object_get_ex(policy_obj, "subSlicePolicy", &sub_policy_obj) &&
           json_object_object_get_ex(sub_policy_obj, "subSlices", &sub_slices_array))
       {

         // Find UE's sub-slice mapping
         char rnti_str[10];
         snprintf(rnti_str, sizeof(rnti_str), "0x%x", iterator->UE->rnti);

         struct json_object *ue_mapping_obj;
         json_object_object_get_ex(policy_obj, "ueSliceMapping", &ue_mapping_obj);

         // Find UE's mapping
         struct json_object *ue_config_obj;
         if (json_object_object_get_ex(ue_mapping_obj, rnti_str, &ue_config_obj))
         {
           int target_sub_slice = json_object_get_int(json_object_object_get(ue_config_obj, "target_sub_slice"));

           // Find matching sub-slice config
           for (int i = 0; i < json_object_array_length(sub_slices_array); i++)
           {
             struct json_object *sub_slice = json_object_array_get_idx(sub_slices_array, i);
             int sub_slice_id = json_object_get_int(json_object_object_get(sub_slice, "sub_slice_id"));

             if (sub_slice_id == target_sub_slice)
             {
               sub_slice_found = true;
             
               // Get sub-slice parameters directly (min_ratio and max_ratio are absolute RB numbers)
               priority = json_object_get_double(json_object_object_get(sub_slice, "priority"));
               min_rb = json_object_get_int(json_object_object_get(sub_slice, "min_ratio"));
               max_rb = json_object_get_int(json_object_object_get(sub_slice, "max_ratio"));
               break;
             }
           }
         }
     ...
     // If no sub-slice found, create default mapping and write to JSON
     // If no sub-slice found, add mapping to existing JSON
     if (!sub_slice_found) {
       // Try to read existing JSON file
       struct json_object *policy_obj = json_object_from_file(filename);
       if (!policy_obj) {
         LOG_E(NR_MAC, "Error reading JSON file %s\n", filename);

         // Use default values for this scheduling round
         priority = 1.5;
         min_rb = 0;
         max_rb = 100;
       ...
       // Get or create ueSliceMapping object
       struct json_object *ue_mapping;
       if (!json_object_object_get_ex(policy_obj, "ueSliceMapping", &ue_mapping)) {
         ue_mapping = json_object_new_object();
         json_object_object_add(policy_obj, "ueSliceMapping", ue_mapping);
       }

       // Add new UE mapping
       struct json_object *ue_config = json_object_new_object();
       json_object_object_add(ue_config, "target_slice", json_object_new_int(1));
       json_object_object_add(ue_config, "target_sub_slice", json_object_new_int(1));
       json_object_object_add(ue_config, "force_slice", json_object_new_boolean(true));

       char rnti_str[10];
       snprintf(rnti_str, sizeof(rnti_str), "0x%x", iterator->UE->rnti);
       json_object_object_add(ue_mapping, rnti_str, ue_config);

       // Write updated JSON back to file
       if (json_object_to_file(filename, policy_obj) == -1) {
         LOG_E(NR_MAC, "Error writing to JSON file %s\n", filename);
       }
     ...
     // Save the original rbSize for debugging
     int original_rbSize = rbSize;

     // Apply sub-slice constraints
     int adjusted_rbSize = (int)(rbSize * priority);

     // Ensure rbSize is within bounds
     if (adjusted_rbSize < min_rb)
     {
       adjusted_rbSize = min_rb;
     }
     else if (adjusted_rbSize > max_rb)
     {
       adjusted_rbSize = max_rb;
     }

     // Update final rbSize
     rbSize = adjusted_rbSize;

       // Added print for debugging sub-slice parameters including the original rbSize
     printf("[Sub-Slice Debug] original rbSize=%d, sub_slice_found=%d, priority=%.2f, min_rb=%d, max_rb=%d, final rbSize=%d\n",
            original_rbSize,
            sub_slice_found,
            priority,
            min_rb,
   ...
   for(int j=0;j< num_slice ;j++){
    ...
    //min_rb_s[sid]= n_rb_sched_s; //update the min_rb array with actual used prbs
    //n_rb_sched_s_tot+=n_rb_sched_s_array[sid];
    //if(n_rb_sched_s_array[sid]) printf("RBs %d for slice %d in slot %d \n", n_rb_sched_s_array[sid],slice_list_tmp[sid],slot);
  }
   if (n_rb_sched_s_tot > n_rb_sched_tot ) AssertFatal(1==0,"PRB Violation \n");
 }
 ...
 static void nr_fr1_dlsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
 {
   ...
   nr_store_dl_slice_info(module_id, frame, slot);
 
   /* Retrieve amount of data to send for this UE */
   //nr_store_dlsch_buffer(module_id, frame, slot);
   nr_store_dlsch_buffer_slice(module_id, frame, slot);
   ...
   // Slice selection for each UE in the current slot
    ...
    // Slice policy update
    if ((slot == 2) && (frame & 127) == 0)  nr_update_slice_policy(module_id, frame, slot);
 ...
 /* proportional fair scheduling algorithm in each slice*/
