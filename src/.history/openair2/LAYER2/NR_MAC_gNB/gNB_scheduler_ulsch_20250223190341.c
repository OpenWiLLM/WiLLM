...
static bool nr_fr1_ulsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  AssertFatal(scc != NULL, "We need one serving cell config common\n");

  // 检查是否有活跃的UE
  bool any_ue_active = false;
  NR_UE_info_t *UE;
  ...
  // 获取第一个有效UE的调度控制和BWP信息 
  NR_UE_sched_ctrl_t *sched_ctrl = NULL;
  NR_UE_UL_BWP_t *current_BWP = NULL;
  
  UE_iterator(nr_mac->UE_info.list, UE) {
    if (UE) {
      sched_ctrl = &UE->UE_sched_ctrl;
      current_BWP = &UE->current_UL_BWP;
      break;
    }
  }

  if (!sched_ctrl || !current_BWP) {
    LOG_E(NR_MAC, "No valid UE found for uplink scheduling\n");
    return false;
  }
  ...
  // 为所有UE设置调度信息
  {
    UE_iterator(nr_mac->UE_info.list, UE1) {
      NR_UE_sched_ctrl_t *ue_sched_ctrl = &UE1->UE_sched_ctrl;
      AssertFatal(K2 == get_K2(tdaList, tda, mu),
                  ...
                  UE1->rnti);
      ue_sched_ctrl->sched_pusch.slot = sched_slot;
      ue_sched_ctrl->sched_pusch.frame = sched_frame;
    }
  }
  ...
  SLIV2SL(tdaList->list.array[tda]->startSymbolAndLength, &startSymbolIndex, &nrOfSymbols);
  const uint16_t symb = SL_to_bitmap(startSymbolIndex, nrOfSymbols);

  // 计算可用PRB数
  int n_rb_sched = bwpSize;
  for (int i = 0; i < bwpSize; i++) {
    if ((vrb_map_UL[bwpStart + i] & symb) != 0) {
      n_rb_sched--;
    }
  }
  ...
  // 检查是否有足够的PRB
  if (n_rb_sched <= 0) {
    LOG_D(NR_MAC, "No available PRBs in UL for frame %d slot %d\n", frame, slot);
    return false;
  }
  ...
  for (int i = 0; i < bwpSize; i++) {
    rballoc_mask[i] = ((vrb_map_UL[bwpStart + i] & symb) == 0) ? symb : 0;
  }
  ...
  // 预估切片PRB
  slice_prb_estimate_ul(module_id,
                       frame,
                       slot,
                       nr_mac->UE_info.list,
                       max_sched_ues,
