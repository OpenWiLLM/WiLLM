...
typedef struct slice_schd_s {
  int sid;
  int prb;
  int allowed_num_prbs;
  int min_prbs;
} slice_schd_t;
...
static void pf_ul(module_id_t module,
  frame_t frame,
  sub_frame_t slot,
...
int compare_s_ul(const void *a, const void *b)
{
  return (((slice_schd_t *)a)->prb - ((slice_schd_t *)b)->prb);
}
...
/* Add these function declarations near the top of the file */
...
void nr_update_slice_policy_ul(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  const char filename[] = "../../../rrmPolicy.json";
  ...
  for (int i = 0; i < json_object_array_length(s_array); i++) {
    ...
    sD_flag = json_object_get_int(json_object_object_get(s_array_obj, "sD_flag"));
  ...
  for (int i = 0; i < mac->dl_num_slice; i++) {
    printf("ULULUL_SLI_info->list[%d]->policy.min_ratio %d, SLI_info->list[%d]->policy.max_ratio %d \n",
           i,
           SLI_info->list[i]->policy.min_ratio,
           i,
           SLI_info->list[i]->policy.max_ratio);
  }
}

void slice_prb_estimate_ul(module_id_t module_id,
                           frame_t frame,
                           sub_frame_t slot,
                           NR_UE_info_t **UE_list,
                           int max_num_ue,
                           ...
                           slice_schd_t *s_array)
{
  ...
  int n_rb_min_total = 0;
  for (int s1 = 0; s1 < num_slice; s1++) {
    n_rb_min_total += s_array[s1].min_prbs;
  }
  int n_rb_remain = n_rb_sched - n_rb_min_total;

  // Init slice list
  for (int s1 = 0; s1 < num_slice; s1++) {
    s_array[s1].sid = s1;
    s_array[s1].prb = 0;
    // s_array[s1].allowed_num_prbs = n_rb_sched;
    int max_prbs = min(n_rb_sched * SLI_info->list[s1]->policy.max_ratio / 100, n_rb_remain);
    s_array[s1].allowed_num_prbs = s_array[s1].min_prbs + max_prbs;
    n_rb_remain -= max_prbs;
    s_array[s1].min_prbs = (n_rb_sched * SLI_info->list[s1]->policy.min_ratio) / 100;

    for (int s2 = 0; s2 < num_slice; s2++) {
      if ((s_array[s1].sid) != s2)
        s_array[s1].allowed_num_prbs -= ((n_rb_sched * SLI_info->list[s2]->policy.min_ratio) / 100);
    }
  }
  // Loop over slices
  for (int s_i = 0; s_i < num_slice; s_i++) {
    int sl_id = s_array[s_i].sid; // slice id for this iteration
    ...
    UE_iterator(UE_list, UE)
    {
      ...
      int8_t harq_id = sched_ctrl->feedback_dl_harq.head;
      if (harq_id < 0) {
        LOG_D(NR_MAC, "[UE %04x][%4d.%2d] no free HARQ process available \n", UE->rnti, frame, slot);
        continue;
      }
      ...
      const NR_mac_dir_stats_t *ul_stats = &UE->mac_stats.ul;
      const double a = 0.0005f; // corresponds to 200ms window
      const uint32_t b = ul_stats->current_bytes;
      UE->ul_thr_ue = (1 - a) * UE->ul_thr_ue + a * b;

      /* Check DL buffer and skip this UE if no bytes and no TA necessary */
      if (sched_ctrl->estimated_ul_buffer == 0) {
        continue;
      }
      ...
      // const uint16_t bwpSize = coresetid == 0 ?mac->cset0_bwp_size : ul_bwp->BWPSize;
      // int rbStart = 0; // start wrt BWPstart
      const NR_mac_dir_stats_t *stats = &UE->mac_stats.ul;
      const NR_bler_options_t *bo = &mac->ul_bler;
      // const int max_mcs_table = ul_bwp->mcs_table == 1 ? 27 : 28;
      // const int max_mcs = min(sched_ctrl->ul_max_mcs, max_mcs_table);
      // uint8_t mcs;

      uint8_t nrOfLayers = 1;
      uint8_t Qm = nr_get_Qm_ul(mac->ul_bler.harq_round_max, ul_bwp->mcs_table);
      uint16_t R = nr_get_code_rate_ul(mac->ul_bler.harq_round_max, ul_bwp->mcs_table);

      int time_domain_allocation;
      NR_tda_info_t *tda_info = malloc(sizeof(NR_tda_info_t));
      time_domain_allocation = get_ul_tda(mac, scc, frame, slot);
      AssertFatal(time_domain_allocation >= 0, "Unable to find PUSCH time domain allocation in list\n");

      const int coresetid = sched_ctrl->coreset->controlResourceSetId;

      *tda_info = get_ul_tda_info(ul_bwp, time_domain_allocation, coresetid, NR_RNTI_C, 0);
      const uint16_t slbitmap = SL_to_bitmap(tda_info->startSymbolIndex, tda_info->nrOfSymbols);

      // Freq-demain allocation
      uint16_t rbStart = 0;
      uint16_t bwpSize = ul_bwp->BWPSize;
      ...
      while (rbStart + max_rbSize < bwpSize && (rballoc_mask[rbStart + max_rbSize] & slbitmap) == slbitmap
             && max_rbSize <= n_rb_max_s)
        max_rbSize++;

      NR_pusch_dmrs_t *dmrs_parms = malloc(sizeof(NR_pusch_dmrs_t));

      *dmrs_parms = get_ul_dmrs_params(scc, ul_bwp, tda_info, nrOfLayers);
      ...
      nr_find_nb_rb(Qm,
                    ...
                    dmrs_parms->N_PRB_DMRS * dmrs_parms->num_dmrs_symb,
                    sched_ctrl->estimated_ul_buffer + oh,
                    min_rbSize,
                    max_rbSize,
                    &TBS,
                    &rbSize);
  ...
  qsort(s_array, num_slice, sizeof(slice_schd_t), compare_s_ul);
}

void nr_store_ul_slice_info(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  UE_iterator(RC.nrmac[module_id]->UE_info.list, UE)
  {
    NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;

    /* Loop over all active UL logical channels */
    for (int i = 0; i < sched_ctrl->dl_lc_num; ++i) {
      const int lcid = sched_ctrl->dl_lc_ids[i];
      const uint16_t rnti = UE->rnti;

      sched_ctrl->dl_sl_info[lcid].nssai_config = mac_rlc_get_nssai(rnti, lcid);
      sched_ctrl->dl_sl_info[lcid].id = 0;

      /* Map NSSAI to slice ID */
      for (int j = 1; j < RC.nrmac[module_id]->dl_num_slice; j++) {
        if ((RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sST == sched_ctrl->dl_sl_info[lcid].nssai_config.sST)
            && (RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sD_flag
                == sched_ctrl->dl_sl_info[lcid].nssai_config.sD_flag)) {
          if (RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sD_flag) {
            if ((RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sD[2] == sched_ctrl->dl_sl_info[lcid].nssai_config.sD[2])
                && (RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sD[1]
                    == sched_ctrl->dl_sl_info[lcid].nssai_config.sD[1])
                && (RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sD[0]
                    == sched_ctrl->dl_sl_info[lcid].nssai_config.sD[0])) {
              sched_ctrl->dl_sl_info[lcid].id = RC.nrmac[module_id]->SLI_info.list[j]->s_id;
            }
          } else {
            sched_ctrl->dl_sl_info[lcid].id = RC.nrmac[module_id]->SLI_info.list[j]->s_id;
          }
    ...
    for (int k = 1; k <= MAX_NUM_PDU_SESSION; k++) {
      if (sched_ctrl->avail_slice_list[k].id > 0)
        sched_ctrl->num_slice_d++;
    }

    if ((slot == 0) && (frame & 127) == 0) {
      LOG_I(NR_MAC, "UL Active slices for UE %x = [", UE->rnti);
      for (int k = 0; k < sched_ctrl->num_slice_d; k++)
        LOG_I(NR_MAC, " %d ", sched_ctrl->avail_slice_list[k].id);
      LOG_I(NR_MAC, "]\n");
    }
  }
}

void nr_store_ulsch_buffer_slice(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  UE_iterator(RC.nrmac[module_id]->UE_info.list, UE)
  {
    NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;
    ...
    for (int i = 0; i < sched_ctrl->dl_lc_num; ++i) {
      ...
      /* Update buffer size for the slice this LC belongs to */
      for (int j = 0; j < sched_ctrl->num_slice_d; j++) {
        if (sched_ctrl->avail_slice_list[j].id == sched_ctrl->dl_sl_info[lcid].id) {
          sched_ctrl->avail_slice_list[j].bytes += sched_ctrl->rlc_status[lcid].bytes_in_buffer;
          break;
        }
      }

      LOG_D(NR_MAC,
            "LCID%1d: ULC LC %d total RLC buffer size = %d bytes, %d PDU \n",
            sched_ctrl->dl_sl_info[lcid].id,
            lcid,
            sched_ctrl->rlc_status[lcid].bytes_in_buffer,
            sched_ctrl->rlc_status[lcid].pdus_in_buffer);
    }

    for (int j = 0; j < sched_ctrl->num_slice_d; j++) {
      LOG_D(NR_MAC,
            "Slice ID %d buffer size = %d bytes\n",
            sched_ctrl->avail_slice_list[j].id,
            sched_ctrl->avail_slice_list[j].bytes);
    }

    if ((slot == 0) && (frame & 127) == 0) {
      for (int k = 0; k < sched_ctrl->num_slice_d; k++) {
        if (sched_ctrl->avail_slice_list[k].bytes > 0) {
          sched_ctrl->slice_for_this_sched = sched_ctrl->avail_slice_list[k].id;
          break;
        }
      }
    }
...
void nr_slice_preprocess_ul(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  UE_iterator(UE_info->list, UE_s)
  {
    ...
    if (sched_ctrl_s->ul_failure)
      continue;

    // Selecting a slice to schedule in this slot :
    // 1. If there is a active Harq for this slice select that
    // 2. No HARQ random selection incase of more than one slice
    ...
    switch (sched_ctrl_s->num_slice_d) {
      ...
      case 1:
        // sched_ctrl_s-> slice_for_this_sched = sched_ctrl_s->active_slice[0];break;
        sched_ctrl_s->slice_for_this_sched = sched_ctrl_s->avail_slice_list[0].id;
        break;
      case 2:
        sched_ctrl_s->slice_for_this_sched = sched_ctrl_s->avail_slice_list[1].id;
        break;
      case 3:

        if ((sched_ctrl_s->avail_slice_list[1].bytes > 0) && (slot % 2 == 0) && (ue_ind % 2 == 0)) {
          sched_ctrl_s->slice_for_this_sched = sched_ctrl_s->avail_slice_list[1].id;
          break;
        }
        if ((sched_ctrl_s->avail_slice_list[2].bytes > 0) && (slot % 2 == 0) && (ue_ind % 2 == 1)) {
          sched_ctrl_s->slice_for_this_sched = sched_ctrl_s->avail_slice_list[2].id;
          break;
        }

        if ((sched_ctrl_s->avail_slice_list[2].bytes > 0) && (slot % 2 == 1) && (ue_ind % 2 == 0)) {
          sched_ctrl_s->slice_for_this_sched = sched_ctrl_s->avail_slice_list[2].id;
          break;
        }
        if ((sched_ctrl_s->avail_slice_list[1].bytes > 0) && (slot % 2 == 1) && (ue_ind % 2 == 1)) {
          sched_ctrl_s->slice_for_this_sched = sched_ctrl_s->avail_slice_list[1].id;
          break;
        }
        break;
                                ...
                                const int tda)
{
  ...
  NR_tda_info_t tda_info = get_ul_tda_info(ul_bwp,
                                           ...
                                           tda);
  bool reuse_old_tda =
      (retInfo->tda_info.startSymbolIndex == tda_info.startSymbolIndex) && (retInfo->tda_info.nrOfSymbols <= tda_info.nrOfSymbols);
  ...
  fill_pdcch_vrb_map(nrmac, CC_id, &sched_ctrl->sched_pdcch, CCEIndex, sched_ctrl->aggregation_level);
  ...
  for (int rb = 0; rb < sched_ctrl->sched_pusch.rbSize; rb++)
    rballoc_mask[rb + sched_ctrl->sched_pusch.rbStart] ^=
        SL_to_bitmap(sched_pusch->tda_info.startSymbolIndex, sched_pusch->tda_info.nrOfSymbols);
  return true;
}

void pf_ul_slice(module_id_t module_id,
                 frame_t frame,
                 sub_frame_t slot,
                 NR_UE_info_t **UE_list,
                 int max_num_ue,
                 ...
                 slice_schd_t *s_array)
{
  ...
  NR_Slices_t *SLI_info = &nrmac->SLI_info;
  UEsched_t UE_sched_slice[MAX_MOBILES_PER_GNB] = {0};
  int n_rb_sched_retrans = 0;

  // 默认切片处理,保证向后兼容
  if (!s_array) {
    pf_ul(module_id, frame, slot, UE_list, max_num_ue, n_rb_sched, rballoc_mask);
    return;
  }
  ...
  // 首先检查是否有可用的PRB资源
  if (n_rb_sched <= 0) {
    LOG_E(NR_MAC, "No PRB available for scheduling\n");
    return;
  }

  // 检查是否有有效的UE列表
  if (!UE_list || !UE_list[0]) {
    LOG_D(NR_MAC, "No UE available for scheduling\n"); 
    return;
  }

  // 检查切片配置是否正确初始化
  if (!s_array) {
    LOG_W(NR_MAC, "No slice configuration, using default scheduling\n");
    pf_ul(module_id, frame, slot, UE_list, max_num_ue, n_rb_sched, rballoc_mask);
    return;
  }
  ...
  // 原有的资源计算逻辑
  int total_min_rb = 0;
  const uint8_t num_slice = RC.nrmac[module_id]->dl_num_slice;
  NR_Slices_t *SLI_info = &RC.nrmac[module_id]->SLI_info;

  // 计算所有切片的最小资源需求
  for (int s_i = 0; s_i < num_slice; s_i++) {
    if (!SLI_info->list[s_i]) {
      LOG_E(NR_MAC, "Slice %d configuration not initialized\n", s_i);
      return;
    }
    s_array[s_i].min_prbs = (n_rb_sched * SLI_info->list[s_i]->policy.min_ratio) / 100;
    total_min_rb += s_array[s_i].min_prbs;
  }

  // 确保总需求不超过可用资源
  if (total_min_rb > n_rb_sched) {
    LOG_W(NR_MAC, 
          "Total minimum PRB requirement %d exceeds available PRBs %d, scaling down\n",
          total_min_rb, n_rb_sched);
    float scale = (float)n_rb_sched / total_min_rb;
    for (int s_i = 0; s_i < num_slice; s_i++) {
      s_array[s_i].min_prbs = (int)(s_array[s_i].min_prbs * scale);
    }
    total_min_rb = n_rb_sched;
  }
  
  // 分配剩余资源
  int remaining_rbs = n_rb_sched - total_min_rb;
  if (remaining_rbs > 0) {
    // 根据最大比例分配剩余资源
    for (int s_i = 0; s_i < num_slice; s_i++) {
      const int max_additional = (n_rb_sched * SLI_info->list[s_i]->policy.max_ratio) / 100 - s_array[s_i].min_prbs;
      if (max_additional > 0) {
        const int allocated = (remaining_rbs < max_additional) ? remaining_rbs : max_additional; // 替换MIN宏
        s_array[s_i].min_prbs += allocated;
        remaining_rbs -= allocated;
      }
    }
  }
  ...
  // 首先处理重传的UE
  UE_iterator(UE_list, UE2)
  {
    NR_UE_sched_ctrl_t *sched_ctrl = &UE2->UE_sched_ctrl;
    if (UE2->Msg4_ACKed != true || sched_ctrl->ul_failure)
      continue;

    int8_t harq_pid = sched_ctrl->feedback_ul_harq.head;
    if (harq_pid < 0) {
      LOG_D(NR_MAC, "RNTI %04x has no UL HARQ process available\n", UE2->rnti);
      continue;
    }

    NR_UE_ul_harq_t *cur_harq = &sched_ctrl->ul_harq_processes[harq_pid];
    LOG_D(NR_MAC, "pf_ul_slice: UE %04x harq_id %d\n", UE2->rnti, harq_pid);

    if (cur_harq->round > 0) {
      NR_sched_pusch_t *sched_pusch = &sched_ctrl->sched_pusch;
      const int tda = get_ul_tda(nrmac, scc, sched_pusch->frame, sched_pusch->slot);

      bool r = allocate_ul_retransmission(nrmac, frame, slot, rballoc_mask, &n_rb_sched, UE2, harq_pid, scc, tda);
      if (!r) {
        LOG_D(NR_MAC, "[UE %04x][%4d.%2d] UL retransmission failed\n", UE2->rnti, frame, slot);
        continue;
      }

      remainUEs--;
      n_rb_sched_retrans += sched_pusch->rbSize;
      LOG_D(NR_MAC, "[UE %04x][%4d.%2d] UL Retransmission scheduled\n", UE2->rnti, frame, slot);
    }
  }

  // 根据切片配置分配最小PRB
  for (int s_i = 0; s_i < num_slice; s_i++) {
    int min_prbs = s_array[s_i].min_prbs;
    if (min_prbs > 0) {
      s_array[s_i].allowed_num_prbs = min_prbs;
      n_rb_sched -= min_prbs;
    }
  }

  // 分配剩余PRB
  int remain_prbs = n_rb_sched;
  for (int s_i = 0; s_i < num_slice; s_i++) {
    if (remain_prbs > 0) {
      int max_allowed = s_array[s_i].allowed_num_prbs;
      int additional = min(remain_prbs, max_allowed - s_array[s_i].min_prbs);
      s_array[s_i].allowed_num_prbs += additional;
      remain_prbs -= additional;
    }
  }
  ...
  for (int s_i = 0; s_i < num_slice; s_i++) {
    ...
    UE_iterator(UE_list, UE)
    {
      ...
      // 跳过不属于当前切片的UE
      if (sched_ctrl->slice_for_this_sched != sid) {
        continue;
      }

      // 计算调度系数
      NR_UE_UL_BWP_t *current_BWP = &UE->current_UL_BWP;
      const uint32_t tbs = ul_pf_tbs[current_BWP->mcs_table][sched_ctrl->sched_pusch.mcs];
      float coeff_ue = (float)tbs / UE->ul_thr_ue;

      // 添加到待调度列表
      UE_sched_slice[curUE].coef = coeff_ue;
      UE_sched_slice[curUE].UE = UE;
      curUE++;
    }

    // 按照调度系数排序
    qsort(UE_sched_slice, curUE, sizeof(UEsched_t), comparator);
    ...
    while (remainUEs > 0 && iterator->UE != NULL) {
      ...
      sched_pusch->tda_info = get_ul_tda_info(ul_bwp,
                                              sched_ctrl->coreset->controlResourceSetId,
                                              sched_ctrl->search_space->searchSpaceType->present,
                                              NR_RNTI_C,
      ...
      int rbStart = 0;
      while (rbStart < ul_bwp->BWPSize && (rballoc_mask[rbStart] & slbitmap) != slbitmap)
        rbStart++;

      sched_pusch->rbStart = rbStart;

      int max_rbSize = min(s_array[s_i].allowed_num_prbs - n_rb_sched_s, min_rb);
      while (rbStart + max_rbSize < ul_bwp->BWPSize && (rballoc_mask[rbStart + max_rbSize] & slbitmap) == slbitmap)
        max_rbSize++;

      if (max_rbSize < min_rb) {
        LOG_D(NR_MAC, "[UE %04x][%4d.%2d] cannot allocate UL data\n", iterator->UE->rnti, frame, slot);
        iterator++;
        continue;
      }

      // 根据切片配置约束rbSize
      sched_pusch->rbSize = max_rbSize;

      // 更新资源使用
      n_rb_sched_s += sched_pusch->rbSize;
      for (int rb = 0; rb < sched_pusch->rbSize; rb++)
        rballoc_mask[rb + sched_pusch->rbStart] ^= slbitmap;

      remainUEs--;
      iterator++;
    }

    s_array[s_i].min_prbs = n_rb_sched_s;
    n_rb_sched_s_tot += n_rb_sched_s;
  }

  // 检查总分配的PRB不超过可用PRB
  if (n_rb_sched_s_tot > n_rb_sched) {
    LOG_E(NR_MAC, "%4d.%2d SLICE PRB allocation %d exceeds total PRBs %d\n", frame, slot, n_rb_sched_s_tot, n_rb_sched);
    return;
  }
}
...
//  corresponding MAC CE or padding as described in Tables 6.2.1-1 and 6.2.1-2 for the DL-SCH and UL-SCH respectively. There is one
//  LCID field per MAC subheader. The LCID field size is 6 bits; L: The Length field indicates the length of the corresponding MAC
//  SDU or variable-sized MAC CE in bytes. There is one L field per MAC subheader except for subheaders corresponding to fixed-sized
//  MAC CEs and padding. The size of the L field is indicated by the F field; F: length of L is 0:8 or 1:16 bits wide R: Reserved
//  bit, set to zero.
  ...
  while (!done && pdu_len > 0) {
    ...
    switch (rx_lcid) {
      ...
      case UL_SCH_LCID_L_TRUNCATED_BSR:
        ...
        n_Lcg = bsr_l->LcgID7 + bsr_l->LcgID6 + bsr_l->LcgID5 + bsr_l->LcgID4 + bsr_l->LcgID3 + bsr_l->LcgID2 + bsr_l->LcgID1
                + bsr_l->LcgID0;
...
void handle_nr_ul_harq(const int CC_idP, module_id_t mod_id, frame_t frame, sub_frame_t slot, const nfapi_nr_crc_t *crc_pdu)
{
                       ...
                       const uint16_t rssi)
{
  ...
  if (UE && UE_waiting_CFRA_msg3 == false) {
    ...
    LOG_D(NR_MAC,
          "[gNB %d][PUSCH %d] CC_id %d %d.%d Received ULSCH sdu from PHY (rnti %04x) ul_cqi %d TA %d sduP %p, rssi %d\n",
          gnb_mod_idP,
          harq_pid,
          CC_idP,
          frameP,
    ...
    for (int i = 0; i < NR_NB_RA_PROC_MAX; ++i) {
      ...
      if (no_sig) {
        ...
        LOG_D(NR_MAC, "reset RA state information for RA-RNTI 0x%04x/index %d\n", ra->rnti, i);
                            ...
                            int *additional_max_tpmi)
{
  ...
  if (num_ue_srs_ports == 2) {
    if (max_rank == 1) {
      if (ul_FullPowerTransmission
          && *ul_FullPowerTransmission == NR_PUSCH_Config__ext1__ul_FullPowerTransmission_r16_fullpowerMode1) {
        max_tpmi = 2;
      } else {
...
#ifdef SRS_IND_DEBUG
            LOG_I(NR_MAC,
                  "(pI %i, gI %i,  uI %i, layer_idx %i) w = %c, channel_matrix --> real %i, imag %i\n",
                  pI,
                  gI,
                  uI,
                  layer_idx,
                  w,
                  channel_matrix16[index].r,
...
#ifdef SRS_IND_DEBUG
        LOG_I(NR_MAC,
              "(pI %i, gI %i) precoded_channel_coef --> real %i, imag %i\n",
              pI,
              gI,
              precoded_channel_matrix_re[index_gI_pI],
              precoded_channel_matrix_im[index_gI_pI]);
#endif
      }
    }

    uint32_t precoded_signal_power =
        calc_power_complex(precoded_channel_matrix_re, precoded_channel_matrix_im, num_prgs * num_gnb_antenna_elements);
                                     ...
                                     uint32_t ulsch_max_frame_inactivity)
{
  ...
  LOG_D(NR_MAC, "%4d.%2d UL inactivity %d slots has_data %d SR %d\n", frame, slot, diff, has_data, sched_ctrl->SR);
                  ...
                  uint16_t *rballoc_mask)
{
  ...
  UE_iterator(UE_list, UE)
  {
    ...
    const bool do_sched =
        nr_UE_is_to_be_scheduled(scc, 0, UE, sched_pusch->frame, sched_pusch->slot, nrmac->ulsch_max_frame_inactivity);
...
static bool nr_fr1_ulsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  const NR_SIB1_t *sib1 = nr_mac->common_channels[0].sib1
                              ? nr_mac->common_channels[0].sib1->message.choice.c1->choice.systemInformationBlockType1
                              : NULL;
  ...
  LOG_D(NR_MAC, "UL %d.%d : start_prb %d, end PRB %d, available PRBs %d\n", frame, slot, st, e, n_rb_sched);
  ...
  nr_store_ul_slice_info(module_id, frame, slot);

  /* Retrieve amount of data to send for this UE */
  // nr_store_dlsch_buffer(module_id, frame, slot);
  nr_store_ulsch_buffer_slice(module_id, frame, slot);
  ...
  // avaliable slices
  nr_slice_preprocess_ul(module_id, frame, slot);

  // slice policy update
  if ((slot == 2) && (frame & 127) == 0)
    nr_update_slice_policy_ul(module_id, frame, slot);
  ...
  slice_prb_estimate_ul(module_id, frame, slot, nr_mac->UE_info.list, max_sched_ues, n_rb_sched, rballoc_mask, bwpSize, s_array);

  /* proportional fair scheduling algorithm */
  pf_ul_slice(module_id, frame, slot, nr_mac->UE_info.list, max_sched_ues, n_rb_sched, rballoc_mask, s_array);
  return true;
}
...
void nr_schedule_ulsch(module_id_t module_id, frame_t frame, sub_frame_t slot, nfapi_nr_ul_dci_request_t *ul_dci_req)
{
  int n_rb_sched = 0;
  gNB_MAC_INST *nr_mac = RC.nrmac[module_id];
  uint8_t num_slice = nr_mac->dl_num_slice;
  slice_schd_t s_array[num_slice];
  ...
  UE_iterator(UE_info->list, UE)
  {
    ...
    LOG_D(NR_MAC,
          "ULSCH/PUSCH: %4d.%2d RNTI %04x UL sched %4d.%2d DCI L %d start %2d RBS %3d startSymbol %2d nb_symbol %2d dmrs_pos %x "
          "MCS Table %2d MCS %2d nrOfLayers %2d num_dmrs_cdm_grps_no_data %2d TBS %4d HARQ PID %2d round %d RV %d NDI %d est %6d "
          "sched %6d est BSR %6d TPC %d\n",
          frame,
          slot,
          rnti,
          sched_pusch->frame,
    ...
    if (future_ul_tti_req->SFN != sched_pusch->frame || future_ul_tti_req->Slot != sched_pusch->slot)
      LOG_W(MAC,
            ...
            sched_pusch->frame,
            sched_pusch->slot);
    AssertFatal(future_ul_tti_req->n_pdus < sizeof(future_ul_tti_req->pdus_list) / sizeof(future_ul_tti_req->pdus_list[0]),
    ...
    if (current_BWP->pusch_servingcellconfig && current_BWP->pusch_servingcellconfig->rateMatching) {
      ...
      pusch_pdu->maintenance_parms_v3.tbSizeLbrmBytes = nr_compute_tbslbrm(current_BWP->mcs_table, bw_tbslbrm, *maxMIMO_Layers);
    } else
    ...
    if (pusch_pdu->transform_precoding == NR_PUSCH_Config__transformPrecoder_enabled) {
      ...
      if ((!NR_DMRS_UplinkConfig || !NR_DMRS_UplinkConfig->transformPrecodingEnabled
           || (!NR_DMRS_UplinkConfig->transformPrecodingEnabled->sequenceGroupHopping
    ...
    pdcch_pdu->numDlDci++;
    dci_pdu->RNTI = rnti;
    if (coreset->pdcch_DMRS_ScramblingID && ss->searchSpaceType->present == NR_SearchSpace__searchSpaceType_PR_ue_Specific) {
    ...
    // Check if total PRBs allocated across all slices exceeds available PRBs
    int n_rb_sched_total = 0;
    for (int s_i = 0; s_i < num_slice; s_i++) {
      n_rb_sched_total += s_array[s_i].min_prbs;
    }
    if (n_rb_sched_total > n_rb_sched) {
      LOG_E(NR_MAC, "%4d.%2d SLICE PRB allocation %d exceeds total PRBs %d\n", frame, slot, n_rb_sched_total, n_rb_sched);
    }
  }
}
