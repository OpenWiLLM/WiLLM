...
static bool nr_fr1_ulsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  // 检查是否有活跃的UE
  NR_UE_info_t *UE_temp = NULL;
  bool any_ue_active = false;
  UE_iterator(nr_mac->UE_info.list, UE_temp) {
    if (UE_temp) {
      any_ue_active = true;
      break; 
    }
  ...
  NR_UE_info_t *first_UE = NULL;
  UE_iterator(nr_mac->UE_info.list, first_UE) {
    if (first_UE) {
      sched_ctrl = &first_UE->UE_sched_ctrl;
      current_BWP = &first_UE->current_UL_BWP;
      break;
    }
  }
