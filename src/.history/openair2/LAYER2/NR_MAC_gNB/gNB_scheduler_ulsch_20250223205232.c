                           ...
                           slice_schd_t *s_array)
{
  ...
  // 检查输入参数的有效性
  if (n_rb_sched <= 0) {
    LOG_E(NR_MAC, "Invalid n_rb_sched %d\n", n_rb_sched);
    return;
  }
  ...
  for (int s1 = 0; s1 < num_slice; s1++) {
    s_array[s1].sid = s1;
    s_array[s1].prb = 0;
    s_array[s1].min_prbs = 0;
    s_array[s1].allowed_num_prbs = 0;
  }
  ...
  for (int s1 = 0; s1 < num_slice; s1++) {
    ...
    total_min_rb += s_array[s1].min_prbs;
  }
  ...
  if (total_min_rb > n_rb_sched) {
    ...
    total_min_rb = 0;
    for (int s1 = 0; s1 < num_slice; s1++) {
      s_array[s1].min_prbs = (int)(s_array[s1].min_prbs * scale);
      total_min_rb += s_array[s1].min_prbs;
    }
  }
  ...
  for (int s1 = 0; s1 < num_slice && remaining_rbs > 0; s1++) {
    int max_additional = (n_rb_sched * SLI_info->list[s1]->policy.max_ratio) / 100 - s_array[s1].min_prbs;
    if (max_additional > 0) {
      int allocated = min(remaining_rbs, max_additional);
      s_array[s1].allowed_num_prbs = s_array[s1].min_prbs + allocated;
      remaining_rbs -= allocated;
    } else {
      s_array[s1].allowed_num_prbs = s_array[s1].min_prbs;
    }
  }
  ...
  for (int s1 = 0; s1 < num_slice; s1++) {
    total_allocated += s_array[s1].allowed_num_prbs;
  }

  if (total_allocated > n_rb_sched) {
    LOG_E(NR_MAC, "Total allocated PRBs %d exceeds available PRBs %d\n", total_allocated, n_rb_sched);
    // 按比例缩减分配
    float scale = (float)n_rb_sched / total_allocated;
    for (int s1 = 0; s1 < num_slice; s1++) {
      s_array[s1].allowed_num_prbs = (int)(s_array[s1].allowed_num_prbs * scale);
    }
  }
}
