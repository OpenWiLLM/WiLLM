                 ...
                 slice_schd_t *s_array)
{
  ...
  // 计算所有切片的最小资源需求
  for (int s_i = 0; s_i < num_slice; s_i++) {
    if (!nrmac->SLI_info.list[s_i]) {
      LOG_E(NR_MAC, "Slice %d configuration not initialized\n", s_i);
      return;
    }
    s_array[s_i].min_prbs = (n_rb_sched * nrmac->SLI_info.list[s_i]->policy.min_ratio) / 100;
    total_min_rb += s_array[s_i].min_prbs;
  }

  // 确保总需求不超过可用资源
  if (total_min_rb > n_rb_sched) {
    LOG_W(NR_MAC, "Total minimum PRB requirement %d exceeds available PRBs %d, scaling down\n", total_min_rb, n_rb_sched);
  ...
  if (remaining_rbs > 0) {
    // 根据最大比例分配剩余资源
    for (int s_i = 0; s_i < num_slice; s_i++) {
      const int max_additional = (n_rb_sched * nrmac->SLI_info.list[s_i]->policy.max_ratio) / 100 - s_array[s_i].min_prbs;
      if (max_additional > 0) {
        const int allocated = (remaining_rbs < max_additional) ? remaining_rbs : max_additional;
