...
void nr_ul_slice_preprocess(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  UE_iterator(ue_info->list, UE) // 空函数没有问题
  {
    ...
    //对原始分配的 RB 数进行约束修正
    if (UE == NULL) {
      LOG_E(NR_MAC, "UE pointer is NULL\n");
      return;
    }
    
    if (rb_alloc < min_rb) {
      if(UE->rnti > 0) {  // 确保 rnti 有效
        LOG_I(NR_MAC,"UE 0x%04x: rb_alloc %d < min_rb %d, adjusting\n",
              UE->rnti, rb_alloc, min_rb);
      } else {
        LOG_E(NR_MAC, "Invalid RNTI value\n");
      }
      rb_alloc = min_rb;  // 恢复这行代码
    }
    else if (rb_alloc > max_rb) { 
      if(UE->rnti > 0) {  // 确保 rnti 有效
        LOG_I(NR_MAC,"UE 0x%04x: rb_alloc %d > max_rb %d, adjusting\n",
