...
void nr_update_slice_policy_ul(module_id_t module_id,
                            frame_t frame,
                            sub_frame_t slot) {
		...
		slice_schd_t *s_array)
{
	...
	for(int s1=0; s1 < num_slice; s1++){
	  ...
	  for(int s2=0; s2 < num_slice; s2++){
		  if((s_array[s1].sid) != s2) s_array[s1].allowed_num_prbs -= ((n_rb_sched* SLI_info->list[s2]->policy.min_ratio )/100);
	  }
	}
	  // Loop over slices  
	...
	/* Loop UE_info->list and allocate new transmission */
      ...
      *tda_info = get_ul_tda_info(ul_bwp,time_domain_allocation,coresetid, NR_RNTI_C, 0);
      ...
      while (rbStart < bwpSize && (rballoc_mask[rbStart] & slbitmap) != slbitmap)  
			...
			*dmrs_parms  = get_ul_dmrs_params(scc, 
									 ul_bwp,
									 tda_info,
									 nrOfLayers);			
...
void nr_store_ul_slice_info(module_id_t module_id,
                           frame_t frame,
                           sub_frame_t slot) {
    ...
    for (int i = 0; i < sched_ctrl->dl_lc_num; ++i) {
      ...
      for(int j = 1; j < RC.nrmac[module_id]->dl_num_slice; j++) {
        if((RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sST == sched_ctrl->dl_sl_info[lcid].nssai_config.sST) && 
           (RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sD_flag == sched_ctrl->dl_sl_info[lcid].nssai_config.sD_flag)) {
          if(RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sD_flag) {
            if((RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sD[2] == sched_ctrl->dl_sl_info[lcid].nssai_config.sD[2]) &&
               (RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sD[1] == sched_ctrl->dl_sl_info[lcid].nssai_config.sD[1]) && 
               (RC.nrmac[module_id]->SLI_info.list[j]->conf.nssai_config.sD[0] == sched_ctrl->dl_sl_info[lcid].nssai_config.sD[0])) {
...
void nr_store_ulsch_buffer_slice(module_id_t module_id,
                                 frame_t frame,
                                 sub_frame_t slot) {
  UE_iterator(RC.nrmac[module_id]->UE_info.list, UE) {
    NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;
    ...
    for (int i = 0; i < sched_ctrl->dl_lc_num; ++i) {
      ...
      LOG_D(NR_MAC, "LCID%1d: ULC LC %d total RLC buffer size = %d bytes, %d PDU \n", 
    ...
    for (int j = 0; j < sched_ctrl->num_slice_d; j++) {
      LOG_D(NR_MAC, "Slice ID %d buffer size = %d bytes\n",
...
void nr_slice_preprocess_ul(module_id_t module_id,
							frame_t  frame,
							sub_frame_t slot){
	...
	UE_iterator(UE_info->list, UE_s) {
		...
		switch(sched_ctrl_s->num_slice_d){
			...
			case 1:
				...
				sched_ctrl_s-> slice_for_this_sched = sched_ctrl_s->avail_slice_list[0].id;break;
			case 2:
				sched_ctrl_s-> slice_for_this_sched = sched_ctrl_s->avail_slice_list[1].id;break;
			case 3:
				...
				if ( (sched_ctrl_s->avail_slice_list[2].bytes > 0) && (slot%2==0) && (ue_ind%2==1) ){
					sched_ctrl_s-> slice_for_this_sched = sched_ctrl_s->avail_slice_list[2].id;break;
				}
...
int compute_ph_factor(int mu, int tbs_bits, int rb, int n_layers, int n_symbols, int n_dmrs, long *deltaMCS)
{
  // 38.213 7.1.1
                       ...
                       const nfapi_nr_crc_t *crc_pdu)
{
  ...
  NR_UE_sched_ctrl_t *sched_ctrl = &UE->UE_sched_ctrl;
  int8_t harq_pid = sched_ctrl->feedback_dl_harq.head;
  ...
  while (crc_pdu->harq_id != harq_pid || harq_pid < 0) {
    ...
    sched_ctrl->harq_processes[harq_pid].is_waiting = false;

    if(sched_ctrl->harq_processes[harq_pid].round >= RC.nrmac[mod_id]->ul_bler.harq_round_max - 1) {
      abort_nr_ul_harq(UE, harq_pid);
    } else {
      sched_ctrl->harq_processes[harq_pid].round++;
      add_tail_nr_list(&sched_ctrl->retrans_ul_harq, harq_pid);
    }
    harq_pid = sched_ctrl->feedback_dl_harq.head;
  }
  remove_front_nr_list(&sched_ctrl->feedback_dl_harq);
  NR_UE_ul_harq_t *harq = &sched_ctrl->ul_harq_processes[harq_pid];
...
*/
void _nr_rx_sdu(const module_id_t gnb_mod_idP,
                       const int CC_idP,
...
int8_t get_max_tpmi(const NR_PUSCH_Config_t *pusch_Config,
...
bool nr_UE_is_to_be_scheduled(const NR_ServingCellConfigCommon_t *scc, int CC_id,  NR_UE_info_t* UE, frame_t frame, sub_frame_t slot, uint32_t ulsch_max_frame_inactivity)
{
				       ...
				       const int tda)
{
  ...
  uint16_t rbStart = 0; // wrt BWP start
  const uint16_t bwpSize = ul_bwp->BWPSize;
  const uint8_t nrOfLayers = retInfo->nrOfLayers;
  LOG_D(NR_MAC,"retInfo->time_domain_allocation = %d, tda = %d\n", retInfo->time_domain_allocation, tda);
  LOG_D(NR_MAC,"tbs %d\n",retInfo->tb_size);
  NR_tda_info_t tda_info = get_ul_tda_info(ul_bwp, sched_ctrl->coreset->controlResourceSetId, sched_ctrl->search_space->searchSpaceType->present, NR_RNTI_C, retInfo->time_domain_allocation);  bool reuse_old_tda = (retInfo->tda_info.startSymbolIndex == tda_info.startSymbolIndex) && (retInfo->tda_info.nrOfSymbols <= tda_info.nrOfSymbols);
		  ...
		  slice_schd_t *s_array)
{
  ...
  // First process retransmissions across all UEs 
  // Slice PRB policy is not applied here. 
  // After retrans, we will apply slice policy on the remaining PRBs
  UE_iterator(UE_list, UE2) {

	NR_UE_sched_ctrl_t *sched_ctrl = &UE2->UE_sched_ctrl;
	
    int8_t harq_id = sched_ctrl->feedback_ul_harq.head;
    if (harq_id < 0) {
    ...
    if (cur_harq->round > 0) {
      NR_sched_pusch_t *NR_sched_pusch = &sched_ctrl->sched_pusch;
      const int tda = get_ul_tda(mac, scc, NR_sched_pusch->frame, NR_sched_pusch->slot);
      bool r = allocate_ul_retransmission(mac, frame, slot, rballoc_mask, &n_rb_sched, UE2, harq_id, scc, tda);
      if (!r) {
        LOG_D(NR_MAC, "[UE %04x][%4d.%2d] UL retransmission allocation failed\n",UE2->rnti, frame, slot);
        continue;
      }
      else { 
        remainUEs--;
        LOG_D(NR_MAC,"[UE %04x][%4d.%2d] UL Retransmission allocated, remainUEs %d\n",UE2->rnti,frame,slot,remainUEs);
      }
    }
  }
  ...
  for(int s_i=0;s_i < num_slice ;s_i++){
	  ...
	  max_rb_s = n_rb_sched_tot;
	  for(int j=0;j< num_slice ;j++){

		  if(s_array[s_i].sid != s_array[j].sid) max_rb_s -= s_array[j].min_prbs;
		  
	  }

	  for(int j=0;j< num_slice ;j++){
		  if (SLI_info->list[j]->s_id == sid){
			  if(max_rb_s > ((n_rb_sched_tot * SLI_info->list[j]->policy.max_ratio)/100)){
				  max_rb_s = (n_rb_sched_tot * SLI_info->list[j]->policy.max_ratio)/100;
			  }
		  }
	  }
	  ...
	  UE_iterator(UE_list, UE) {
	    ...
	    int8_t harq_id = sched_ctrl->harq_processes->round;

		// This UE already got a UL retransmission, skip it for new tx 
        if( harq_id < 0 || // no free HARQ process 
        	sched_ctrl->harq_processes->round != 0)
        {
        	continue;	
        }


	    /* Check whether slice this UE is selecting is the one in this iteration */
      ...
      sched_pusch->mcs = 9;
		...
		/* Check UL Buffer and skip this UE if no bytes and no TA necessary */  
		if (sched_ctrl->estimated_ul_buffer == 0 && frame != (sched_ctrl->ta_frame + 10) % 1024){
			continue;
		}
        ...
        LOG_D(NR_MAC,"[UE %04x][%4d.%2d] tbs %d, ul_thr_ue %f, coeff_ue %f\n",
          UE->rnti,
          frame,
          slot, 
          tbs,
	...
	while (remainUEs> 0 && n_rb_remain_s >= min_rbSize && n_rb_sched_s <= max_rb_s - n_rb_sched_retrans &&  iterator->UE != NULL ){
		...
		NR_UE_ul_harq_t *cur_harq = &sched_ctrl->harq_processes[harq_id];
		...
		sched_pusch->mcs = 9; // start with fixed mcs
		update_ul_ue_R_Qm(sched_pusch->mcs, ul_bwp->mcs_table, ul_bwp->pusch_Config, &sched_pusch->R, &sched_pusch->Qm);
		
		sched_pusch->time_domain_allocation = get_ul_tda(mac, scc, sched_pusch->frame, sched_pusch->slot);
		AssertFatal(sched_pusch->time_domain_allocation>=0,"Unable to find PUSCH time domain allocation in list\n");

		const int coresetid = sched_ctrl->coreset->controlResourceSetId;
    sched_pusch->tda_info = get_ul_tda_info(ul_bwp,sched_pusch->time_domain_allocation, coresetid, NR_RNTI_C, 0);

		NR_tda_info_t *tda_info = &sched_pusch->tda_info;

		const uint16_t slbitmap = SL_to_bitmap(tda_info->startSymbolIndex, tda_info->nrOfSymbols);

		// Freq-demain allocation
		uint16_t rbStart = 0;
    const uint16_t bwpSize = iterator->UE->current_UL_BWP.BWPSize;
    while (rbStart < bwpSize && (rballoc_mask[rbStart] & slbitmap) != slbitmap)  
		  rbStart++;
		...
		while (rbStart + max_rbSize < bwpSize && (rballoc_mask[rbStart + max_rbSize] & slbitmap) == slbitmap 
				&&  max_rbSize <= n_rb_remain_s) 
			max_rbSize++;

		if (rbStart + min_rbSize >= bwpSize || max_rbSize < min_rbSize) {
		...
		const int B = cmax(sched_ctrl->estimated_ul_buffer, 0);
		/* adjust rbSize and MCS according to PHR and BPRE */
		nr_ue_max_mcs_min_rb(ul_bwp->scs, 0, sched_pusch, ul_bwp, min_rbSize, B, &max_rbSize, &sched_pusch->mcs);
		...
		nr_find_nb_rb(sched_pusch->Qm,
					  ...
					  sched_pusch->nrOfLayers,
					  tda_info->nrOfSymbols,
					  1, // DMRS overhead: 6 or 4 DMRS per RB 
					  B,
					  min_rbSize,
					  max_rbSize,
					  &TBS,
		...
		n_rb_remain_s-= sched_pusch->rbSize;
	...
	for(int j=0;j< num_slice ;j++){
		  if(s_array[j].sid == sid) {
			  s_array[j].min_prbs = n_rb_sched_s;
			  n_rb_sched_s_tot+=s_array[j].min_prbs;
		  }
	}

 }
  // Final check to ensure we haven't exceeded total PRBs
  if (n_rb_sched_s_tot > n_rb_sched_tot) {
    LOG_E(NR_MAC, "%4d.%2d SLICE PRB allocation %d exceeds total PRBs %d\n", frame, slot, n_rb_sched_s_tot, n_rb_sched_tot);
    return;
  }
}
...
bool nr_fr1_ulsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  // slice policy update
  if ((slot == 2) && (frame & 127) == 0)  nr_update_slice_policy_ul(module_id, frame, slot);
...
void nr_schedule_ulsch(module_id_t module_id, frame_t frame, sub_frame_t slot, nfapi_nr_ul_dci_request_t *ul_dci_req)
{
  ...
  if (is_xlsch_in_slot(nr_mac->ulsch_slot_bitmap[slot / 64], slot)) {
    ...
    const int CC_id = 0;
    uint16_t *vrb_map_UL =
        &RC.nrmac[module_id]->common_channels[CC_id].vrb_map_UL[ul_buffer_index(frame, slot, *scc->ssbSubcarrierSpacing, nr_mac->UL_tti_req_ahead_size)];
    const uint16_t bwpSize = RC.nrmac[module_id]->UE_info.list[0]->current_UL_BWP.BWPSize;
    uint16_t rballoc_mask[bwpSize];
    int n_rb_sched = 0;
    for (int i = 0; i < bwpSize; i++) {
      // calculate mask: init with "NOT" vrb_map:
      // if any RB in vrb_map is blocked (1), the current RBG will be 0
      rballoc_mask[i] = (~vrb_map_UL[i]) & 0x3fff; // bitwise not and 14 symbols
      if (rballoc_mask[i] == 0x3fff)
        n_rb_sched++;
    }

    /* Calculate the number of free PRBs per slice based on min_ratio, max_ratio */
    slice_prb_estimate_ul(module_id, frame, slot,
                      RC.nrmac[module_id]->UE_info.list, 
                      MAX_MOBILES_PER_GNB,
                      n_rb_sched, rballoc_mask, 
                      bwpSize, s_array);

    /* PF scheduling per slice */
    pf_ul_slice(module_id,
          frame,
          slot,
          RC.nrmac[module_id]->UE_info.list, 
